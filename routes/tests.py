...
class RoutesTestCase(TestCase):
    ...
    def test_model_city_duplicate(self):  # функция проверки дублирования назваия городов
        try:
            a_city = City(name='A')
            a_city.full_clean()
    ...
    def test_model_train_duplicate(self):
        ...
        try:
            ...
            self.assertEqual(
                {'__all__': ['Измените время в пути']},
                e.message_dict
            )

    def test_home_routes_view(self):
        response = self.client.get(reverse('home'))
        self.assertEqual(200, response.status_code)  # проверяем статус код страницы
        self.assertTemplateUsed(response, template_name='routes/home.html')  # проверка шаблона страницы
        self.assertEqual(routes_views.home, response.resolver_match.func)  # проверка ф-и home,
        # используется ли она для отображения

    def test_cbv_detail(self):  # тестируем отображение на основе классов
        response = self.client.get(reverse('city:detail', kwargs={'pk': self.city_A.id}))  # отправляем
        # запрсо на детализацию, присутсвуют аргументы
        self.assertEqual(200, response.status_code)
        self.assertTemplateUsed(response, template_name='cities/details.html')
        self.assertEqual(
            cities_views.CityDetailView.as_view().__name__,
            response.resolver_match.func.__name__
        )  # проверка класса отображения, на использование его для отображения

    def test_find_all_routes(self):
        graph = routes_views.get_graph()
        all_ways = list(routes_views.dfs_paths(
            graph,
            self.city_A.id,
            self.city_E.id)
        )
    ...
    def test_valid_form(self):  # проверка формы ввода даныых маршрута
    ...
    def test_messages_error_more_time(self):
        response = self.client.post('/find/',
